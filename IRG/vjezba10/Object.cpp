...
void Object::draw(const glm::mat4& view, const glm::mat4& projection, const glm::vec3* eye, const Light* light, const Material* material) const {
    ...
    if (light)
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader->ID, "lightView"), 1, GL_FALSE,
            glm::value_ptr(light->view));

        glUniformMatrix4fv(glGetUniformLocation(shader->ID, "lightProjection"), 1, GL_FALSE,
    ...
    if (material)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, material->diffTex);
    }

    mesh->draw();
}

void Object::drawDepth(Shader* depthShader, const glm::mat4& lightSpaceMatrix) const {
    depthShader->use();
