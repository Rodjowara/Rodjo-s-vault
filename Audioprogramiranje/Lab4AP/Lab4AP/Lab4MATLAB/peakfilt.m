...
% primijenuje peaking filter na ulazni signal  x.
...
else c = (tan(pi*Wb/2)-V0) / (tan(pi*Wb/2)+V0); 
% cut
end; 
d = -cos(pi*Wc); 
xh = [0, 0]; %početno stanje spremnika linije
for n = 1:length(x) 
xh_new = x(n) - d*(1-c)*xh(1) + c*xh(2); 
y1 = -c * xh_new + d*(1-c)*xh(1) + xh(2); 
xh = [xh_new, xh(1)]; 
y(n) = 0.5 * H0 * (x(n) - y1) + x(n); 
end;
