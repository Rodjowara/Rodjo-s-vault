...
const int ADDRESS = 0x24;

#define OPERATION_NOP 0
#define OPERATION_POWER 1
#define OPERATION_SQRT 2
...
bool on_i2c_connect(void *user_ctx, uint32_t address, bool read) {
  ...
  //}
  printf("I2C_3 Process connected\n");
...
uint8_t on_i2c_read(void *user_ctx) {
  ...
  if (chip->operation == OPERATION_POWER){
      rez.f = chip->par1.f *chip->par1.f;
  }else if (chip->operation == OPERATION_SQRT){
...
void on_i2c_disconnect(void *user_ctx) {
  // Do nothing
  printf("I2C_3 Process disconected!!!\n");
}
